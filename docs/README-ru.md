...
- **Миграция HTTP клиента**: Обновлен с `aiohttp` на `httpx` для лучшей производительности и поддержки HTTP/2
- **Новый источник API**: Добавлен НБРБ (Национальный банк Беларуси) API как основной источник для фиатных валют
- **Улучшения производительности**: Более быстрые API запросы и лучшее управление ресурсами
- **Современные зависимости**: Использование последних и наиболее эффективных Python библиотек
...
- **Курсы в реальном времени**: Использует НБРБ API для фиатных валют и CurrencyFreaks/ExchangeRate для криптовалют
...
- **JSON база данных**: Простое и портативное хранение данных пользователей
...
Затем отредактируйте файл `.env` с вашими реальными значениями:
```env
...
- **НБРБ**: Основной API для фиатных валют (официальные курсы Национального банка Беларуси)
- **CurrencyFreaks**: Запасной API для фиатных валют, основной для криптовалют
- **ExchangeRate-API**: Запасной API для фиатных валют и криптовалют
...
Бот автоматически применяет оптимизации производительности:
- **Unix/Linux/macOS**: Использует `uvloop` для быстрого event loop
- **Windows**: Использует `WindowsProactorEventLoopPolicy` для лучшей производительности
- **HTTP Client**: Использует `httpx` для быстрых и эффективных API запросов

## 💾 Хранение данных

Бот использует простой JSON файл (`users.json`) для хранения настроек и предпочтений пользователей:
...
**Преимущества JSON хранения:**
- ✅ Нет зависимостей от внешних баз данных
...
- **HTTP клиент**: httpx (современный, быстрый, поддержка HTTP/2)
- **Сервис валют**: Мульти-API с умными запасниками
- **База данных**: JSON файл для пользовательских настроек
...
1. **НБРБ API** (Основной для фиатных валют)
   - Официальные курсы Национального банка Беларуси
   - Бесплатный доступ
   - Курсы в реальном времени

2. **CurrencyFreaks API** (Запасной для фиатных, основной для криптовалют)
   - Курсы в реальном времени
   - 150+ валют
   - Обновления каждые 5 минут

3. **ExchangeRate-API** (Запасной)
   - Резервный источник
   - 170+ валют
